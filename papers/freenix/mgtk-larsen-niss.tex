...
beginning the \gtk developers have paid attention on making feasible
and pratical to make bindings for other programming languages than C.
...
\begin{description}
\item[Class type] A base class like \classname{Super} in
...
\item[Subtyping/Inheritance] For a subclass like \classname{SubClassA}
...
type for the methos !foo! from class \classname{SubSubClassB} has the
type:
\begin{SMLcode}
...
Now, continuing our running example from Section~\ref{sec:example},
the two widgets \textit{window} and \textit{button} gets the following
types
\begin{displaymath}
...
elided).  The first type tells us, reading from left to right, that
...
\begin{itemize}
\item There is another bind from \sml to \gtk called !SML-GTK! which
  can be found at: 
  \begin{center}
  ...
  specific to the SML/NJ compiler.  Also the !SML-GTK! does not handle
  memory management.


\item !gtk+hs! is language binding for Haskell to \gtk 
  \begin{center}
    \url{http://www.cse.unsw.edu.au/~chak/haskell/gtk}
  \end{center}

\item !lablgtk! is language binding for O'Caml to \gtk 
  \begin{center}
    \url{http://wwwfun.kurims.kyoto-u.ac.jp/soft/olabl/lablgtk.html}
  \end{center}

  O'Caml is a ML dialect different from \sml which (among other
  things) has object-orianted features. This binding is different from
  \mgtk because it uses the object oriented features.
...
\item ``Calling Hell from Heaven'' \cite{Finne:1999:CallingHellFromHeaven}

\item ml-nlffi \cite{Blume:2001:nlffi}
...
possible to make an interface from \sml to \gtk.  This is interesting
because \sml is not for serveral reasons: first, the \sml community
gets access to a graphical library which is sourly needed; secord,
that is possible to make an \sml binding to \gtk really tests the
claimed ``interfaceability'' of \gtk because \sml is so radically
different from C in abstraction level and paradigm; finally, we think
that we have demonstrated that it is possible to bind the whole \gnome
platform using mainly machine generated code.

...
is sorely missing such a library.  Second, on a long term basis, when
we have wrapped the whole GNOME platform, it is possible for \sml
programmers to make real applications without having to invent add-hoc
solutions to many standard problems, like as database access, for
...
but important, marked of teaching languages.  Second, as \sml is a
...
Section~\ref{sec:example} shows a ``Hello World'' example using
\mgtk.  Section~\ref{sec:encoding-classes} explains how a
single-inheritance class-hierarchy, in particular \gtk's, can be
encoded in \sml's type system, while retaining type safety.
Section~\ref{sec:process} describes the woking process we have used to make
the \mgtk library.  Section~\ref{sec:mgtk-binding} describes practical
matters of \mgtk, such as which \sml compilers are supported.
Finally, Section~\ref{sec:related-work} lists related work and
...
is the type abbrivation that specifies that \classname{Label} inherits
...
!Super! for the class \classname{Widget} and !GtkBasis! for the dummy

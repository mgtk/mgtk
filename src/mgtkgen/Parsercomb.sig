...
sig
    ...
    type pos = int * int
    val withPos    : ('elm, 'res) parser -> ('elm, pos * 'res) parser
end
...
(* 
   ...
   [withPos par] succeeds with ((c1,c2),r) if par succeeds with r
   and the element(s) consumed by par begins on character position c1
   in the stream and ends on c2.
*)

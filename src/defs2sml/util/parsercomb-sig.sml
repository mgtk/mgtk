...
sig
    ...
    val unless     : ('elm, 'a) parser * ('elm, 'b) parser 
                     -> ('elm, 'a) parser
...
(* 
   ...
   [par1 |> v] succeeds with v if par succeeds on the stream.
   ...
   [par1 unless par2] succeeds with SOME(r) if par1 succeeds with r
   and par2 does not succeed; fails otherwise.

structure Name :> NAME = struct
	...
	let fun loop [] = []
	      ...
	      | loop ("accel"::"group"::xs) = ("accelgroup") :: loop xs
	      | loop ("Action"::"Group"::xs) = ("ActionGroup") :: loop xs
	      | loop ("action"::"group"::xs) = ("actiongroup") :: loop xs
	      | loop ("Icon"::"Set"::xs) = ("IconSet") :: loop xs
	      | loop ("icon"::"set"::xs) = ("iconset") :: loop xs
	      | loop ("Icon"::"Info"::xs) = ("IconInfo") :: loop xs
	      | loop ("icon"::"info"::xs) = ("iconinfo") :: loop xs
	      | loop ("Text"::"View"::xs) = ("TextView") :: loop xs
	      | loop ("text"::"view"::xs) = ("textview") :: loop xs
	      | loop ("Text"::"Attributes"::xs) = ("TextAttributes") :: loop xs
	      | loop ("text"::"attributes"::xs) = ("textattributes") :: loop xs
	      | loop ("Text"::"Child"::"Anchor"::xs) = ("TextChildAnchor") :: loop xs
	      | loop ("text"::"child"::"anchor"::xs) = ("textchildanchor") :: loop xs
	      | loop ("Text"::"Mark"::xs) = ("TextMark") :: loop xs
	      | loop ("text"::"mark"::xs) = ("textmark") :: loop xs
	      | loop ("Text"::"Tag"::"Table"::xs) = ("TextTagTable") :: loop xs
	      | loop ("text"::"tag"::"table"::xs) = ("texttagtable") :: loop xs
	      | loop ("Text"::"Tag"::xs) = ("TextTag") :: loop xs
	      | loop ("text"::"tag"::xs) = ("texttag") :: loop xs
	      | loop ("Tree"::"Model"::xs) = ("TreeModel") :: loop xs
	      | loop ("tree"::"model"::xs) = ("treemodel") :: loop xs
	      | loop ("Tree"::"Path"::xs) = ("TreePath") :: loop xs
	      | loop ("tree"::"path"::xs) = ("treepath") :: loop xs
	      ...
	      | loop ("rc"::"style"::xs) = ("rcstyle") :: loop xs
	      | loop ("Selection"::"Data"::xs) = ("SelectionData") :: loop xs
	      | loop ("selection"::"data"::xs) = ("selectiondata") :: loop xs
	      | loop ("Menu"::"Shell"::xs) = ("MenuShell") :: loop xs
	      | loop ("menu"::"shell"::xs) = ("menushell") :: loop xs
	      | loop ("Tool"::"Item"::xs) = ("ToolItem") :: loop xs
	      | loop ("tool"::"item"::xs) = ("toolitem") :: loop xs
	      | loop (x::xs) = x :: loop xs
	in  loop words end
...
*)
	...
	fun ify [] acc = rev acc
	  ...
	  | ify ("TextView"::rest) acc = ify rest ("View"::"Text"::acc)
	  | ify ("iconset"::rest) acc = ify rest ("set"::"icon"::acc)
	  | ify ("IconSet"::rest) acc = ify rest ("Set"::"Icon"::acc)
	  | ify ("iconinfo"::rest) acc = ify rest ("info"::"icon"::acc)
	  | ify ("IconInfo"::rest) acc = ify rest ("Info"::"Icon"::acc)
	  | ify ("textchildanchor"::rest) acc = ify rest ("anchor"::"child"::"text"::acc)
	  | ify ("TextChildAnchor"::rest) acc = ify rest ("Anchor"::"Child"::"Text"::acc)
	  | ify ("textmark"::rest) acc = ify rest ("mark"::"text"::acc)
	  | ify ("TextMark"::rest) acc = ify rest ("Mark"::"Text"::acc)
	  | ify ("texttagtable"::rest) acc = ify rest ("table"::"tag"::"text"::acc)
	  | ify ("TextTagTable"::rest) acc = ify rest ("Table"::"Tag"::"Text"::acc)
	  | ify ("texttag"::rest) acc = ify rest ("tag"::"text"::acc)
	  | ify ("TextTag"::rest) acc = ify rest ("Tag"::"Text"::acc)
	  ...
	  | ify ("TreeView"::rest) acc = ify rest ("View"::"Tree"::acc)
	  | ify ("treepath"::rest) acc = ify rest ("path"::"tree"::acc)
	  | ify ("TreePath"::rest) acc = ify rest ("Path"::"Tree"::acc)
	  | ify ("treeselection"::rest) acc = ify rest ("selection"::"tree"::acc)
	  | ify ("TreeSelection"::rest) acc = ify rest ("Selection"::"Tree"::acc)
	  | ify ("progressbar"::rest) acc = ify rest ("bar"::"progress"::acc)
	  | ify ("ProgressBar"::rest) acc = ify rest ("Bar"::"Progress"::acc)
	  | ify ("rcstyle"::rest) acc = ify rest ("style"::"rc"::acc)
	  | ify ("RcStyle"::rest) acc = ify rest ("Style"::"Rc"::acc)
	  | ify ("TextAttributes"::rest) acc = ify rest ("Attributes"::"Text"::acc)
	  | ify ("textattributes"::rest) acc = ify rest ("attributes"::"text"::acc)
	  | ify ("SelectionData"::rest) acc = ify rest ("Data"::"Selection"::acc)
	  | ify ("selectiondata"::rest) acc = ify rest ("data"::"selection"::acc)
	  | ify ("MenuShell"::rest) acc = ify rest ("Shell"::"Menu"::acc)
	  | ify ("menushell"::rest) acc = ify rest ("shell"::"menu"::acc)
	  | ify ("ToolItem"::rest) acc = ify rest ("Item"::"Tool"::acc)
	  | ify ("toolitem"::rest) acc = ify rest ("item"::"tool"::acc)
	  | ify ("ActionGroup"::rest) acc = ify rest ("Group"::"Action"::acc)
	  | ify ("actiongroup"::rest) acc = ify rest ("group"::"action"::acc)
